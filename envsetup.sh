...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^uber_") ; then
        UBER_BUILD=$(echo -n $1 | sed -e 's/^uber_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $UBER_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        UBER_BUILD=
    fi
    export UBER_BUILD
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Uber GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/uber/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/uber/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
function repopick() {
    T=$(gettop)
    $T/vendor/uber/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $UBER_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.uber.device | grep -q "$UBER_BUILD");
    then
        ...
        echo "The connected device does not appear to be $UBER_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.uber.device | grep -q "$UBER_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
. $ANDROID_BUILD_TOP/vendor/uber/build/envsetup.sh
